...
> **Note:** `DurableObjectId` and `DurableObjectNamespace` are Cloudflare runtime types provided as ambient globals by `worker-configuration.d.ts` (generated by `wrangler types`). No import is needed — run `pnpm run cf-typegen` to (re)generate the file from your `wrangler.jsonc` bindings.
