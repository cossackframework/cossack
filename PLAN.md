...
- [ ] **1.2** No method-name allowlist on RPC — clients can invoke `bootstrap`, `setProperty`, `proxyClientMethods`, etc. Files: `framework/src/router.ts:514`, `transports/http.ts:67`, `core/src/shared/cossack.ts:625` (`executeAction`). Validate `action` against `cossack:server-methods` metadata; reject `_`-prefixed/builtins.
...
- [ ] **2.2** Duplicate constructor → syntax error breaks the whole client bundle when a class has both `@Server` + `constructor` — `:756-784`.
...
- [ ] **2.5** SSR 500 leaks raw `err.stack` into HTML (XSS + info disclosure) — `framework/src/router.ts:462`.
