 ...
 *
 * - \`auth.middleware\` (registered in src/config/middlewares.ts) populates
 ...
 * `src/config/middlewares.ts`, so this layout only renders children.
 */
...
function db(c: Context) {
  ...
  if (!d) throw new Error('Database not available — register dbMiddleware in src/config/middlewares.ts.');
  return d;
}
