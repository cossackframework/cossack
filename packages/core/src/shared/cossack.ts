...
export abstract class Cossack<Env = any, T extends CossackOptions = {}> extends CossackElement {
    ...
    public __cossackAssertNotRendering(methodName?: string): void {
        ...
        const msg =
            ...
            ' was called during render(). Server methods return Promises that ' +
    ...
    public _render(): string {
        ...
        // Mark the render window so a @Server method invoked from inside
        // render() is caught: the client stubs generated by the security plugin
        // call `this.__cossackAssertNotRendering(...)`, which throws in dev /
        // warns in prod. Without this a server call would return a Promise the
        // synchronous renderer stringifies as "[object Promise]".
        enterRender();
        let template: any;
        try {
            template = this._getWrappedTemplate();
        } finally {
            exitRender();
        }
    ...
    protected async performUpdate() {
        ...
        try {
            ...
            if (shouldUpdate) {
                ...
                // Guard the synchronous render call so server functions invoked
                // during render() trip the runtime guard (see `_render`).
                enterRender();
                let template: any;
                try {
                    template = this._getWrappedTemplate();
