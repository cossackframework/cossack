...
// In production builds, the manifest is available after the client build has
// run. In dev mode with the Cloudflare Vite plugin, there is no manifest on
// disk. We import it lazily at request time so the SSR module can be evaluated
// before the manifest exists (e.g. during a production build where SSR is
// bundled before the client manifest is emitted).
...
async function getManifest(): Promise<Record<string, any>> {
  ...
  try {
    ...
    resolved = manifestModule.default || manifestModule;
  } catch {
