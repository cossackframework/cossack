...
import { repeat, classMap, styleMap } from './directives';

describe('Directives', () => {
    
    it('repeat renders items', () => {
        const items = [{id: 1, text: 'A'}, {id: 2, text: 'B'}];
        const template = html`<ul>${repeat(items, (i) => i.id, (i) => html`<li>${i.text}</li>`)}</ul>`;
    ...
    it('repeat handles keyed updates (simulated)', () => {
         ...
         const template1 = html`${repeat(items, i => i.id, i => i.text)}`;
         
         const container = document.createElement('div');
         render(template1, container);
         expect(container.textContent).toBe('A');
         
         const items2 = [{id: 2, text: 'B'}, {id: 1, text: 'A-Upd'}];
         const template2 = html`${repeat(items2, i => i.id, i => i.text)}`;
