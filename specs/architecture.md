...
-   **Database (`@cossackframework/database`)**: Optional database layer. Re-exports [Kysely](https://kysely.dev) and ships custom dialects for **Cloudflare D1** and **Turso/libSQL** (written against Kysely 0.29's `Dialect` interface — the community `kysely-d1`/`kysely-libsql` packages are stale). Exposes a per-request client via `createDbMiddleware` + a global `db()` helper (AsyncLocalStorage), plus Kysely Migrator-backed `cossack migration` / `cossack seeder` CLI commands. The framework stays decoupled — global request middleware (including `dbMiddleware`) is loaded automatically from `src/bootstrap/middlewares.ts` via `virtual:cossack-middlewares`; the `@cossackframework/database` package is imported by user code, never by the framework.
